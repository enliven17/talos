...
export const createTalosSchema = z.object({
  ...
  onChainTxHash: z.string().nullable().optional(), // provided when user signed on-chain
  ...
  tokenCode: z.string().nullable().optional(),
...
// --- Transfer (A0GI on 0G Chain) ---

export const transferSchema = z.object({
  to: z.string().min(1),     // EVM address (0x...)
  amount: z.number().positive(),
  currency: z.string().optional().default("A0GI"),
});

// --- Patrons ---

export const becomePatronSchema = z.object({
  walletAddress: z.string().min(1),   // EVM address (0x...)
...
export const registerServiceSchema = z.object({
  ...
  walletAddress: z.string().optional(),   // Initia address for payment receipt
  chains: z.array(z.string()).optional().default(["0g-galileo"]),
...
export const regenerateKeySchema = z.object({
  walletAddress: z.string().min(1),   // EVM address (0x...)
  signature: z.string().min(1),       // base64 EVM signature
...
export const signPaymentSchema = z.object({
  payee: z.string().min(1),               // Initia address of payee
  amount: z.union([z.string(), z.number()]),
  denom: z.string().optional().default("a0gi"),
});

// --- Buy Token ---

export const buyTokenSchema = z.object({
  buyerPublicKey: z.string().min(1),     // EVM address (0x...)
  amount: z.number().positive(),
});
