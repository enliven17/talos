...
export function TalosDetailClient({ talos }: { talos: TalosDetail }) {
  ...
  const handleRequestService = useCallback(async () => {
    ...
    try {
      const payee = talos.service.walletAddress || talos.agentWalletAddress;
      if (!payee) {
        alert("No payment address configured for this agent");
        setServiceStatus("error");
        return;
      }

      // User signs + broadcasts A0GI transfer on 0G Chain via MetaMask
      const txHash = await sendTransactionAsync({
        to: payee as `0x${string}`,
        value: parseEther(String(talos.service.price)),
      });

      let payload: Record<string, unknown> = {};
      try { payload = servicePayload.trim() ? JSON.parse(servicePayload) : {}; } catch { payload = { request: servicePayload }; }

      // Submit job with on-chain tx hash for server-side verification
      const res = await fetch(`/api/talos/${talos.id}/service`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ txHash, payload }),
      });
  ...
  }, [address, talos.id, talos.service, talos.agentWalletAddress, servicePayload, sendTransactionAsync]);
  ...
  const [distPreview, setDistPreview] = useState<{
    ...
    breakdown: { walletAddress: string; pulseAmount: number; sharePercent: string; estimatedInit: string }[];
  } | null>(null);
    ...
    try {
      const res = await fetch(`/api/talos/${talos.id}/revenue/buyback`, {
        ...
        body: JSON.stringify({
          ...
          initAmount: Number(buybackForm.usdcAmount),
  ...
  const handleBuyToken = useCallback(async () => {
    ...
    try {
      ...
      if (!recipient) {
        alert("No agent wallet address configured");
        setBuyStatus("error");
        return;
      }
      ...
      const txHash = await sendTransactionAsync({
        ...
        value: parseEther(String(buyCost)),
      });
  ...
  }, [address, buyQty, buyCost, talos.id, talos.agentWalletAddress, sendTransactionAsync]);
            ...
            <div className="bg-surface border border-border p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-foreground mt-1">&gt; {talos.approvalThreshold} A0GI</p>
                </div>
                <div>
            ...
            <div className="bg-surface border border-border p-6">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground">0G Galileo Testnet</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted">Chain ID</span>
                  <span className="text-foreground font-mono">16602</span>
                </div>
                <div className="flex justify-between gap-2">
                  <span className="text-muted shrink-0">Token Code</span>
                  <span className="text-foreground font-mono truncate text-right">
                    {talos.tokenCode || talos.tokenSymbol || "—"}
                  </span>
                </div>
                {talos.agentWalletAddress && (
                  <div className="flex justify-between">
                    <span className="text-muted">Agent Wallet</span>
                    <span className="text-foreground font-mono truncate max-w-[60%]">{talos.agentWalletAddress.slice(0, 6)}...{talos.agentWalletAddress.slice(-4)}</span>
                  </div>
                )}
                {talos.onChainId != null && (
                  <div className="flex justify-between">
                    <span className="text-muted">Registry ID</span>
                    <span className="text-foreground font-mono">#{talos.onChainId}</span>
                  </div>
                )}
                {talos.ensName && (
                  <div className="flex justify-between items-start">
                    <span className="text-muted shrink-0">ENS Name</span>
                    <div className="text-right">
                      <a
                        href={`https://app.ens.domains/${talos.ensName}?chain=sepolia`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-accent font-mono hover:underline block"
                      >
                        {talos.ensName}
                      </a>
                      {talos.ensTxHash && (
                        <a
                          href={`https://sepolia.etherscan.io/tx/${talos.ensTxHash}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-muted text-[10px] hover:text-accent"
                        >
                          tx: {talos.ensTxHash.slice(0, 10)}… ↗
                        </a>
                      )}
                    </div>
                  </div>
                ...
                {talos.agentWalletAddress && (
                  <div className="space-y-1.5 mt-2">
                    <a
                      href={`https://chainscan-galileo.0g.ai/address/${talos.agentWalletAddress}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      View Wallet on 0G Explorer &rarr;
                    </a>
                    {talos.onChainTxHash && (
                      <a
                        href={`https://chainscan-galileo.0g.ai/tx/${talos.onChainTxHash}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block text-center border border-border py-1.5 text-muted hover:text-accent hover:border-accent transition-colors text-[10px]"
                      >
                        Registry TX on 0G &rarr;
                      </a>
                    )}
                    {talos.ensTxHash && (
                      <a
                        ...
                        className="block text-center border border-border py-1.5 text-muted hover:text-accent hover:border-accent transition-colors text-[10px]"
                      >
                        ENS Registration TX on Sepolia &rarr;
                      </a>
                    )}
                  </div>
                )}
          ...
          {talos.service ? (
            <>
              ...
              <div className="bg-surface border border-border p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-foreground mt-1">A0GI on 0G Chain</p>
                  </div>
                  <div>
              ...
              {isConnected ? (
                <div className="bg-surface border border-accent/20 p-6">
                  ...
                  <p className="text-xs text-muted mb-4">
                    Submit a job to this agent. Your A0GI payment is sent on-chain and the agent processes your request.
                  </p>
                  <button
                  ...
                  >
                    {talos.agentOnline ? `Request — ${talos.service.price} A0GI` : "Agent Offline"}
                  </button>
                </div>
              ) : (
              ...
              <div className="bg-surface border border-border p-6">
                ...
                <p className="text-xs text-muted mb-4">
                  Integrate programmatically — pay A0GI on-chain, then call the service endpoint.
                </p>
                <div className="bg-background border border-border p-4 text-xs text-foreground overflow-x-auto font-mono space-y-1">
                  <div className="text-green-400"># 1. Sign A0GI payment (agent-to-agent)</div>
                  <div className="text-muted">POST /api/talos/:requester_id/sign</div>
                  <div className="text-muted">{"{"} payee: &quot;{talos.service.walletAddress.slice(0, 12)}...&quot;, amount: {talos.service.price} {"}"}</div>
                  <div className="mt-3 text-green-400"># 2. Submit job with X-PAYMENT header</div>
                  <div className="text-muted">POST /api/talos/{talos.id}/service</div>
                  <div className="mt-1">Authorization: Bearer &lt;api_key&gt;</div>
                  <div>X-PAYMENT: initia &lt;payment_token&gt;</div>
                  <div className="mt-1">{"{"}</div>
                  <div className="pl-4">&quot;payload&quot;: {"{"} &quot;request&quot;: &quot;your task here&quot; {"}"}</div>
                  <div>{"}"}</div>
                  <div className="mt-3 text-green-400"># 3. Poll for result (async mode)</div>
                ...
                <div className="space-y-1">
                  {distPreview.breakdown.map((b) => (
                    <div key={b.walletAddress} className={`flex items-center justify-between text-xs py-1.5 border-b border-border last:border-0 ${b.walletAddress === address ? "text-accent" : "text-muted"}`}>
                      ...
                      <span className="font-bold">{b.estimatedInit} A0GI</span>
                    </div>
                  ))}
                </div>
                {isConnected && (
                  ...
                  >
                    {distLoading ? "Distributing..." : `Distribute ${distPreview.distributableAmount.toFixed(4)} A0GI to Holders`}
                  </button>
                )}
              </div>
            ) : (
            ...
            <div key={a.id} className="bg-surface border border-border p-5">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  ...
                  {a.amount && <p className="text-xs text-accent mt-1">{Number(a.amount).toFixed(6)} A0GI</p>}
                </div>
              ...
              <div className="space-y-4">
                ...
                {isConnected && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-muted block mb-1">A0GI to spend</label>
                      <input
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface border border-border p-6">
            ...
            <div className="bg-background border border-border p-4 text-xs text-foreground space-y-1 overflow-x-auto font-mono">
              ...
              <div className="text-muted break-all">Token:     {talos.tokenCode || talos.tokenSymbol}</div>
              <div className="text-muted">Network:   0G Galileo Testnet (ChainID 16602)</div>
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <div className="bg-surface border border-border p-4 space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-foreground">{talos.pulsePrice} A0GI</span>
                  </div>
                  <div className="flex justify-between text-xs">
                  ...
                  <div className="flex justify-between text-sm pt-2 border-t border-border">
                    ...
                    <span className="text-accent font-bold">{buyCost.toFixed(6)} A0GI</span>
                  </div>
                </div>
                ...
                >
                  {buyStatus === "buying"
                    ? "Processing..."
                    : buyQty > 0
                      ? `Buy ${buyQty.toLocaleString()} ${talos.tokenSymbol} for ${buyCost.toFixed(6)} A0GI`
                      : `Enter amount to buy`}
                </button>

                <p className="text-xs text-muted/50 text-center">
                  0G Galileo testnet &mdash; real transaction, real tokens
                </p>
              </div>
            )}
          </div>
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <div className="bg-surface border border-border p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-foreground">0G Chain · A0GI</span>
                  </div>
                </div>
                ...
                >
                  {serviceStatus === "paying" ? "Processing payment..." : `Pay ${talos.service.price} A0GI & Submit Job`}
                </button>

                <p className="text-xs text-muted/50 text-center">
                  A0GI is transferred on-chain before job creation
                </p>
              </div>
            )}
          </div>
