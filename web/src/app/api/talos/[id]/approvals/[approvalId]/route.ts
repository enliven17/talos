...
import { recordApprovalOnChain } from "@/lib/og-chain";
...
export async function PATCH(
  ...
  try {
    ...
    if (!decidedBy) {
      return Response.json(
        { error: "decidedBy (EVM address 0x...) is required" },
        { status: 400 },
      );
    }
    ...
    // Record approval decision on-chain (0G Chain)
    const onChainResult = await recordApprovalOnChain(approvalId, id, status);
