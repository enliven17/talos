...
// POST /api/talos/:id/transfer — Execute A0GI transfer on 0G Chain
export async function POST(
  ...
  try {
    ...
    if (!agentSecret) {
      return Response.json(
        { error: "Agent private key not configured for this TALOS" },
        { status: 503 },
      );
    }

    const result = await sendA0GI(agentSecret, to, String(amount));
    return Response.json({
      status: "completed",
      currency: "A0GI",
      to,
