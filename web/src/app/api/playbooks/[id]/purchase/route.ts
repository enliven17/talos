...
import { verifyA0GIPayment, broadcastA0GIPayment } from "@/lib/og-chain";

// POST /api/playbooks/:id/purchase — Purchase a playbook via 0G Chain A0GI payment
export async function POST(
  ...
  try {
    ...
    // Verify A0GI payment
    const isValid = await verifyA0GIPayment(paymentToken, expectedPayee, String(playbook.price));
    if (!isValid) {
      return Response.json({ error: "Invalid or insufficient payment" }, { status: 402 });
    }

    // Broadcast payment on-chain
    let txHash: string;
    try {
      const result = await broadcastA0GIPayment(paymentToken);
      txHash = result.txHash;
    } catch (err) {
      console.error("Playbook purchase broadcast failed:", err);
    ...
    await db.insert(tlsRevenues).values({
      ...
      currency: playbook.currency ?? "A0GI",
