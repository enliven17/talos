...
export type Transaction = {
  ...
  timestamp: string;
  txHash: string | null;
  via: "axl" | "http" | null; // communication channel
};
...
export async function fetchActivityTransactions(
  ...
  for (const j of jobs) {
    const requesterId = j.requesterTalosId ?? "";
    // Derive a human-readable buyer label when the requester is a wallet address or "web"
    let buyerName = j.buyerName;
    if (!buyerName) {
      if (!requesterId || requesterId === "web") {
        buyerName = "Web User";
      } else if (requesterId.startsWith("init1")) {
        buyerName = `${requesterId.slice(0, 8)}...${requesterId.slice(-4)}`;
      } else {
        buyerName = "Unknown";
      }
    }
    transactions.push({
      ...
      via: (j.payload as Record<string, unknown> | null)?.axl ? "axl" : "http",
    });
  }
