...
export const tlsTalos = pgTable(
  "tls_talos",
  {
    ...
    onChainId: integer("onChainId").unique(),        // 0G Chain registry TALOS ID
    ...
    // Pulse Token (ERC-20 on 0G Chain)
    tokenCode: text("tokenCode"),                    // ERC-20 contract address for Pulse token
    ...
    // EVM Wallet Addresses (0x... format — 0G Chain)
    ...
    // Agent EVM Wallet (private key stored server-side as TALOS_AGENT_SECRET_{id})
    ...
    // ENS agent identity (e.g. "vega.talos.eth")
    ensName: text("ensName").unique(),
    ensTxHash: text("ensTxHash"),         // Sepolia ENS registration tx hash
    onChainTxHash: text("onChainTxHash"), // 0G Galileo registry tx hash
...
export const tlsRevenues = pgTable(
  "tls_revenues",
  {
    ...
    currency: text("currency").notNull().default("A0GI"),
...
export const tlsCommerceServices = pgTable(
  "tls_commerce_services",
  {
    ...
    chains: text("chains").array().notNull().default(["0g-galileo"]),
...
// ─── Commerce Job (Initia Payment Job Queue) ──────────────────────
...
export const tlsCommerceJobs = pgTable(
  "tls_commerce_jobs",
  {
    ...
    result: jsonb("result"),
    status: text("status").notNull().default("pending"),
    paymentSig: text("paymentSig").unique(),   // 0G Chain payment token hash (replay prevention)
    txHash: text("txHash"),                    // 0G Chain transaction hash after broadcast
    amount: numeric("amount", { precision: 18, scale: 6 }).notNull(),
...
export const tlsPlaybookPurchases = pgTable(
  "tls_playbook_purchases",
  {
    ...
    buyerPublicKey: text("buyerPublicKey").notNull(),   // EVM address (0x...)
