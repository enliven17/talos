...
async function main() {
  ...
  const [talos] = await db
    ...
    .values({
      ...
      gtmBudget: "180",
      creatorPublicKey: "GBCV01ALICE000000000000000000000000000000000000000000000AB",
      onChainId: 108,
      agentName: "community-voice",
      agentWalletId: "GBCV01AGENTKEY0000000000000000000000000000000000000000WCVT",
      agentWalletAddress: "GBCV01AGENTKEY0000000000000000000000000000000000000000WCVT",
      agentOnline: true,
    })
    .returning();
  ...
  await db.insert(tlsPatrons).values([
    { talosId: talos.id, walletAddress: "GBCV01ALICE000000000000000000000000000000000000000000000AB", role: "Creator", pulseAmount: 240000, share: "0" },
    { talosId: talos.id, walletAddress: "GBCV02GOVERNOR0000000000000000000000000000000000000000GV", role: "Governor", pulseAmount: 120000, share: "0" },
    { talosId: talos.id, walletAddress: "GBCV03INVESTOR0000000000000000000000000000000000000000IV", role: "Investor", pulseAmount: 120000, share: "0" },
    { talosId: talos.id, walletAddress: "GBCV04CONTRIB0000000000000000000000000000000000000000CT", role: "Contributor", pulseAmount: 60000, share: "0" },
    { talosId: talos.id, walletAddress: "GBCV05ADVISOR0000000000000000000000000000000000000000AV", role: "Advisor", pulseAmount: 60000, share: "0" },
  ]);
  ...
  await db.insert(tlsCommerceServices).values({
    ...
    price: "0.012",
    walletAddress: "GBCV01ALICE000000000000000000000000000000000000000000000AB",
    chains: ["0g"],
    fulfillmentMode: "instant",
  });
  console.log("  service: product_review @ 0.012 A0GI");
  ...
  await db.insert(tlsApprovals).values([
    ...
    },
    {
      talosId: talos.id,
      type: "transaction",
      title: "$18 A0GI — Purchase Scout Trend data for review topic selection",
  ...
  await db.insert(tlsRevenues).values([
    { talosId: talos.id, amount: "0.012", source: "product_review", currency: "A0GI" },
    { talosId: talos.id, amount: "0.012", source: "product_review", currency: "A0GI" },
    { talosId: talos.id, amount: "0.012", source: "product_review", currency: "A0GI" },
  ]);
  console.log("  3 revenues ($0.036 A0GI)");
