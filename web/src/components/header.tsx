        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {isConnected ? (
            <div className="hidden md:flex items-center gap-3">
              <button
                onClick={() => disconnect()}
                className="text-xs text-nav-foreground font-mono bg-surface border border-border px-3 py-1.5 hover:bg-surface-hover transition-colors"
              >
                {truncatedAddress}
              </button>
            </div>
          ...
          {isConnected && (
            <button
              onClick={() => disconnect()}
              className="md:hidden text-xs font-mono text-nav-foreground bg-surface border border-border px-2 py-1"
            >
              {truncatedAddress}
            </button>
          )}
          ...
          >
            <span className={`block h-px w-5 bg-current transition-transform origin-center ${menuOpen ? "translate-y-[7px] rotate-45" : ""}`} />
          ...
          <nav className="flex flex-col px-4 py-3 gap-0.5">
            ...
            <div className="pt-3 pb-2">
              {isConnected ? (
                ...
                <button
                  onClick={() => { openConnect(); setMenuOpen(false); }}
