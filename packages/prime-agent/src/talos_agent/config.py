...
class Settings(BaseSettings):
    ...
    # Talos Web API
    talos_api_url: str = "https://talos-0g.vercel.app"
    talos_api_key: str = ""
    talos_id: str = ""
    ...
    # 0G Compute (preferred — decentralized AI inference on 0G)
    og_compute_api_key: str = ""
    og_compute_model: str = "qwen3-235b-a22b"
    og_compute_api: str = "https://api.0g.ai"
    ...
    def llm_api_key(self) -> str:
        return self.og_compute_api_key or self.groq_api_key or self.openai_api_key

    @property
    def llm_model(self) -> str:
        if self.og_compute_api_key:
            return self.og_compute_model
        return self.groq_model if self.groq_api_key else self.openai_model

    @property
    def llm_base_url(self) -> str | None:
        if self.og_compute_api_key:
            return f"{self.og_compute_api}/v1"
        return "https://api.groq.com/openai/v1" if self.groq_api_key else None

    # 0G Storage
    og_storage_rpc: str = "https://indexer-storage-testnet-standard.0g.ai"
    og_operator_private_key: str = ""

    # Gensyn AXL — P2P agent communication
    axl_enabled: bool = False
    axl_port: int = 9002
    axl_binary: str = "axl"
