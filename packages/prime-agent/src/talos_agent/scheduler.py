...
async def run(settings: Settings, agent_slot: int = 0) -> None:
    ...
    axl_client = None
    if settings.axl_enabled:
        from talos_agent.axl import AXLClient
        axl_client = AXLClient(
            port=settings.axl_port,
            binary_path=settings.axl_binary,
            talos_id=settings.talos_id,
        )

    # Build tools (AXL client injected now; started below before first cycle)
    tools = build_all_tools(api=api, db=db, browser=browser, settings=settings, axl_client=axl_client)
    console.print(f"[green]Registered {len(tools)} tools.[/green]")

    # Start Gensyn AXL node (P2P agent communication — best-effort)
    if axl_client is not None:
        axl_started = await axl_client.start()
        if axl_started:
            console.print(f"[green]AXL P2P node ready — peer ID: {axl_client.peer_id}[/green]")
            if talos_config.get("service"):
                await axl_client.announce_service(
                    service_name=talos_config["service"].get("serviceName", ""),
                    price=float(talos_config["service"].get("price", 0)),
                    description=talos_config["service"].get("description", ""),
                    wallet_address=talos_config.get("agentWalletAddress", ""),
                )
        else:
            console.print("[yellow]AXL not available — P2P tools disabled[/yellow]")
            axl_client = None
        ...
        while not shutdown_event.is_set():
            async with agent_lock:
                ...
                try:
                    ...
                    db.add_activity("research", "Agent cycle started", "system")
                    await agent_loop(
                        settings=settings,
                        tools=tools,
                    ...
                    db.add_activity("research", "Agent cycle completed", "system")

                    # Checkpoint state to 0G Storage after every cycle
                    try:
                        total_rev = float(context.performance_summary.get("total_revenue", 0))
                        og_result = await api.store_og_state(
                            settings.talos_id,
                            cycle_count=_cycle_count,
                            total_revenue=total_rev,
                        )
                        if og_result and og_result.get("rootHash"):
                            console.print(
                                f"[dim cyan][0G] State checkpointed: {og_result['rootHash'][:16]}…[/dim cyan]"
                            )
                    except Exception:
                        pass  # 0G Storage is best-effort

                except Exception as e:
                    console.print(f"[red]Agent cycle error: {e}[/red]")
                    db.add_activity("research", f"Agent cycle error: {e}", "system")
            try:
