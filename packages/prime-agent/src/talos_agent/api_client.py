...
class TalosAPIClient:
    ...
    async def report_revenue(
        ...
        r = await self._client.post(
            ...
            json={"amount": amount, "currency": "INIT", "source": source, "txHash": tx_hash},
        )
        if r.status_code in (200, 201):
            return r.json()
        return None
    ...
    async def store_og_state(
        ...
        *,
        ...
        """Checkpoint agent state to 0G Storage via Web API."""
        payload = {
            "talosId": talos_id,
            "type": "state",
            "data": {
        ...
        r = await self._client.post("/api/og-storage", json=payload)
        if r.status_code in (200, 201):
            return r.json()
        return None

    async def store_og_memory(
        self,
        talos_id: str,
        *,
        content: str,
        ...
        """Append a memory entry to 0G Storage via Web API."""
        payload = {
            "talosId": talos_id,
            "type": "memory",
            "data": {
