...
async def talos_purchase(
    ...
    """Buy a service from another Talos via Initia payment.
    ...
    if response.status_code == 402:
        ...
        # Step 2: Sign the payment via Initia wallet
        sig = await client.sign_payment(payee=payee, amount=price)
        ...
        return {
            ...
            "amount": price,
            "message": f"Service purchased for {price} INIT. Job ID: {result.get('id')}",
        }
...
async def talos_submit_result(
    ...
    return {
        ...
        "job_id": job_id,
        "revenue": amount,
        "message": f"Job completed. Earned {amount} INIT.",
    }
...
async def talos_status() -> dict[str, Any]:
    ...
    return {
        ...
        "total_revenue_init": total_revenue,
